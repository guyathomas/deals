...
const { scrapeSite, createBrowser } = require('./scraper');
...
async function runScrape(db, { siteKey } = {}) {
  ...
  const needsLocalBrowser = targetSites.some((s) => !s.proxy);
  let browser;
  const results = [];

  try {
    if (needsLocalBrowser) browser = await createBrowser();
    ...
    for (const site of targetSites) {
      ...
      try {
        const products = await scrapeSite(browser, effectiveSite, { extractProducts, normalizeProducts });
